 ...
 */
void KateHighlighting::doHighlight ( YZLine *prevLine,
                                     YZLine *textLine,
...
void KateHighlighting::getKateExtendedAttributeList (const QString &schema, QList<KateExtendedAttribute::Ptr> &list)
{
  ...
  config->setGroup("Default Item Styles - Schema " + schema );
#warning port me
...
#warning port me
      ...
      tmp=s[1]; if (!tmp.isEmpty()) {
         col=tmp.toUInt(0,16); p->setForeground(YZColor(col)); }

      tmp=s[2]; if (!tmp.isEmpty()) {
         col=tmp.toUInt(0,16); p->setSelectedForeground(YZColor(col)); }
      ...
      tmp=s[7]; if (!tmp.isEmpty()) {
         col=tmp.toUInt(0,16); p->setBackground(YZColor(col)); }

      tmp=s[8]; if (!tmp.isEmpty()) {
         col=tmp.toUInt(0,16); p->setSelectedBackground(YZColor(col)); }
#endif
...
#warning port me
#if 0
    if (!color.isEmpty()) newData->setForeground(YZColor(color));
    if (!selColor.isEmpty()) newData->setSelectedForeground(YZColor(selColor));
    ...
    if (!bgColor.isEmpty()) newData->setBackground(YZColor(bgColor));
    if (!selBgColor.isEmpty()) newData->setSelectedBackground(YZColor(selBgColor));
#endif
...
{
#warning port me ?
  static const QString& sq = QString("\"'");
...
{
  QMutableHashIterator< QString, QList<KateExtendedAttribute::Ptr> > it = m_attributeArrays;
  while (it.hasNext())
  {
    it.next();
    ...
    QList<KateExtendedAttribute::Ptr>& array = it.value();
    array.clear();

    for (uint z = 0; z < nAttribs; z++)
    {
      KateExtendedAttribute::Ptr itemData = itemDataList.at(z);
      KateExtendedAttribute::Ptr newAttribute( new KateExtendedAttribute(*defaultStyleList.at(itemData->defaultStyleIndex())) );
