...
void YzisSchemaConfigColorTab::readConfig (const QString& config)
{
  ...
  YZSession::me->getOptions()->setGroup( config );
  m_back->setColor(YZSession::me->getOptions()->readQColorEntry("Color Background", tmp0));
  m_selected->setColor(YZSession::me->getOptions()->readQColorEntry("Color Selection", tmp1));
  m_current->setColor(YZSession::me->getOptions()->readQColorEntry("Color Highlighted Line", tmp2));
  m_bracket->setColor(YZSession::me->getOptions()->readQColorEntry("Color Highlighted Bracket", tmp3));
  m_wwmarker->setColor(YZSession::me->getOptions()->readQColorEntry("Color Word Wrap Marker", tmp4));
  m_tmarker->setColor(YZSession::me->getOptions()->readQColorEntry("Color Tab Marker", tmp5));
  m_iconborder->setColor(YZSession::me->getOptions()->readQColorEntry("Color Icon Bar", tmp6));
  m_linenumber->setColor(YZSession::me->getOptions()->readQColorEntry("Color Line Number", tmp7));
  ...
  for (int i = 0; i < KTextEditor::MarkInterface::reservedMarkersCount(); i++)
  {
    ...
    m_markerColors[i] = YZSession::me->getOptions()->readQColorEntry(QString("Color MarkType%1").arg(i + 1), mark[i]);
    QPixmap pix(16, 16);
...
{
  YZSession::me->getOptions()->setGroup( config );
  YZSession::me->getOptions()->setQColorEntry("Color Background", m_back->color());
  YZSession::me->getOptions()->setQColorEntry("Color Selection", m_selected->color());
  YZSession::me->getOptions()->setQColorEntry("Color Highlighted Line", m_current->color());
  YZSession::me->getOptions()->setQColorEntry("Color Highlighted Bracket", m_bracket->color());
  YZSession::me->getOptions()->setQColorEntry("Color Word Wrap Marker", m_wwmarker->color());
  YZSession::me->getOptions()->setQColorEntry("Color Tab Marker", m_tmarker->color());
  YZSession::me->getOptions()->setQColorEntry("Color Icon Bar", m_iconborder->color());
  YZSession::me->getOptions()->setQColorEntry("Color Line Number", m_linenumber->color());

#if KDE_IS_VERSION( 3,3,0 )
  for (int i = 0; i < KTextEditor::MarkInterface::reservedMarkersCount(); i++)
  {
    YZSession::me->getOptions()->setQColorEntry(QString("Color MarkType%1").arg(i + 1), m_markerColors[i]);
  }
#endif
  YZSession::me->getOptions()->setGroup( "Global" );
}
...
void YzisSchemaConfigFontTab::readConfig (const QString& config)
{
  ...
  YZSession::me->getOptions()->setGroup( config );
  m_fontchooser->setFont ( YZSession::me->getOptions()->readQStringEntry("Font", f.toString()) );
...
{
  YZSession::me->getOptions()->setGroup( config );
  YZSession::me->getOptions()->setQStringEntry("Font", myFont.toString() );
  YZSession::me->getOptions()->setGroup( "Global" );
}
...
/*DISABLED FOR NOW mik ...  QPalette p ( m_defaultStyles->palette() );
  ...
  YZSession::me->getOptions()->setGroup( YZSession::me->schemaManager()->schema(schema) );
  p.setColor( QColorGroup::Base, YZSession::me->getOptions()->readQColorEntry( "Color Background", _c ) );
  _c = QColor("darkblue");//XXX? KGlobalSettings::highlightColor();
  p.setColor( QColorGroup::Highlight, YZSession::me->getOptions()->readQColorEntry( "Color Selection", _c ) );
...
/*DISABLE FOR NOW ... mik  QPalette p ( m_styles->palette() );
  QColor _c ( "black" );//XXX? KGlobalSettings::baseColor() );
  YZSession::me->getOptions()->setGroup(YZSession::me->schemaManager()->schema(m_schema) );
  p.setColor( QColorGroup::Base, YZSession::me->getOptions()->readQColorEntry( "Color Background", _c ) );
  _c = QColor("darkblue");//XXX? KGlobalSettings::highlightColor();
  p.setColor( QColorGroup::Highlight, YZSession::me->getOptions()->readQColorEntry( "Color Selection", _c ) );
